...
function mountRocketTodoWidget(initialState: RocketTodoState): MountedRocketTodo {
  ...
  header.style.cssText = "padding:5.5px 5.5px 4.4px; border-bottom:1px solid #efeff0;";

  const heading = document.createElement("h2");
  heading.textContent = "Today";
  heading.style.cssText = "margin:0; font-size:14.3px; line-height:1; font-weight:700; letter-spacing:-0.03em;";

  const subheading = document.createElement("p");
  subheading.textContent = "Rocket Launch Tasks 5";
  subheading.style.cssText = "margin:3.3px 0 0; color:#5f646d; font-size:5.5px; font-weight:600;";

  const list = document.createElement("ul");
  list.style.cssText = "margin:0; padding:0; list-style:none; overflow:auto; border-radius:5.5px;";
  ...
  state.items.forEach((item) => {
    ...
    row.style.cssText =
      "display:grid; grid-template-columns:auto 1fr auto; align-items:center; gap:4.4px; padding:5.5px 5.5px; border-bottom:1px solid #efeff0; background:#ffffff;";
    ...
    label.style.cssText = "display:block; font-size:5.5px; line-height:1.3; color:#262a33;";

    const meta = document.createElement("span");
    meta.textContent = "Required before launch";
    meta.style.cssText = "display:block; margin-top:1.1px; color:#66a443; font-size:5.5px;";

    const category = document.createElement("span");
    category.textContent = CATEGORY_LABEL_BY_ID[item.id] ?? "Launch #";
    category.style.cssText = "font-size:5.5px; color:#80858f; white-space:nowrap;";
    ...
    const applyCheckedStyle = () => {
      ...
      checkbox.style.backgroundSize = "5.5px 5.5px";
...
export function setupRocketTodoRenderer() {
  ...
  KritzelDynamicObjectRendererRegistry.register(ROCKET_TODO_RENDERER_KEY, {
  ...
  return () => {
    ...
    KritzelDynamicObjectRendererRegistry.unregister(ROCKET_TODO_RENDERER_KEY);
  };
}
...
export function createRocketTodoCustomElement() {
  ...
  const customElement = new KritzelDynamicObject({
    element: placeholder,
