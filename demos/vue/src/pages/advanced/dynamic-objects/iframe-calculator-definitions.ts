import type { KritzelIframeContent } from '@kritzel/vue-editor'

export interface RuntimeIframeDefinition {
  name: string
...
const calculatorCss = `
  ...
  #mode-label { color: #167a55; font-size: 12px; font-weight: 800; text-transform: uppercase; }
  ...
  .equals { grid-row: span 2; background: #42b883; border-color: #42b883; color: #fff; }
  ...
  .toggle button.active { background: #fff; color: #167a55; box-shadow: 0 1px 4px rgba(32, 33, 36, .12); }
...
const calculatorScript = `
  ...
  const operation = (left, right, operator) => operator === 'add' ? left + right : operator === 'subtract' ? left - right : operator === 'multiply' ? left * right : right === 0 ? 0 : left / right;
...
export const mockComponentDefinitions: RuntimeIframeDefinition[] = [
  {
    ...
    content: { html: calculatorHtml('Simple Calculator', false, false), css: calculatorCss, script: calculatorScript },
    loadingContent,
    initialState: { displayValue: '0', storedValue: null, pendingOperator: null, shouldReplaceDisplay: true, mode: 'simple' },
  },
  {
    name: 'Scientific calculator',
    content: { html: calculatorHtml('Scientific Calculator', true, false), css: calculatorCss, script: calculatorScript },
    loadingContent,
  ...
  },
  {
    ...
    content: { html: calculatorHtml('Adaptive Calculator', true, true), css: calculatorCss, script: calculatorScript },
    loadingContent,
    initialState: { displayValue: '0', storedValue: null, pendingOperator: null, shouldReplaceDisplay: true, mode: 'simple' },
  },
]