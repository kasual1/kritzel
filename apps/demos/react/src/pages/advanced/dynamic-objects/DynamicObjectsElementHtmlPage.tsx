...
export function DynamicObjectsElementHtmlPage() {
  const editorRef = useRef<HTMLKritzelEditorElement | null>(null);
  const hasAddedInitialDynamicObject = useRef(false);
  const mountedWidgets = useRef<Map<string, MountedHtmlWidget>>(new Map());

  useEffect(() => {
    KritzelDynamicObjectRendererRegistry.register(HTML_RENDERER_KEY, {
      onMount: ({ object, container, data }) => {
        if (!container) {
          return;
        }

        const previous = mountedWidgets.current.get(object.id);
        if (previous) {
          previous.destroy();
          previous.root.remove();
          mountedWidgets.current.delete(object.id);
        }
        ...
        mountedWidgets.current.set(object.id, mountedWidget);
      },
      onUnmount: ({ object, container }) => {
        const mountedWidget = mountedWidgets.current.get(object.id);
        if (!mountedWidget) {
          return undefined;
        }
    ...
    return () => {
      mountedWidgets.current.forEach((mountedWidget) => {
        mountedWidget.destroy();
        mountedWidget.root.remove();
      });
      mountedWidgets.current.clear();
      KritzelDynamicObjectRendererRegistry.unregister(HTML_RENDERER_KEY);
    };
  }, []);
  ...
  return (
    <div style={hostStyle}>
      <KritzelEditor
        ref={editorRef}
        editorId="react-dynamic-objects-html"
        theme="light"
