...
class Observable {
   ...
   */
  on (name, f) {
    setIfUndefined$1(this._observers, name, create$7).add(f);
  }
...
const writeTerminatedString = (encoder, str) =>
  writeTerminatedUint8Array(encoder, encodeUtf8(str));
 ...
 */
const assert$1 = property => { if (!property) throw create$5('Assert failed') };
...
const errorUnexpectedEndOfArray = create$5('Unexpected end of array');
const errorIntegerOutOfRange = create$5('Integer out of Range');
 ...
 */
const wait = timeout => create$4((resolve, _reject) => setTimeout(resolve, timeout));
 ...
 * @return {boolean}
 */
const equalityFlat = (a, b) => a === b || (a != null && b != null && a.constructor === b.constructor && ((isArray$1(a) && equalFlat$1(a, /** @type {Array<T>} */ (b))) || (typeof a === 'object' && equalFlat(a, b))));
 ...
 * @param {Uint8Array} buf
 */
const toHexString = buf => map$2(buf, b => b.toString(16).padStart(2, '0')).join('');
...
const shapeExtends = (a, b) => {
  ...
  if (isArray$1(a)) {
    return every$1(a, aitem =>
      some$1(b, bitem => shapeExtends(aitem, bitem))
    )
  } else if (isObject(a)) {
...
const assert = production
  ...
  : (o, schema) => {
      ...
      if (!schema.check(o, err)) {
        throw create$5(`Expected value to be of type ${schema.constructor.name}.\n${err.toString()}`)
      }
    };
/* c8 ignore end */
 ...
 * @param {EventListener} f
 */
const removeEventListener$1 = (el, name, f) => el.removeEventListener(name, f);
...
const removeEventListeners = (node, listeners) => {
  forEach(listeners, (name, f) => removeEventListener$1(node, name, f));
  return node
};
...
class Doc extends ObservableV2 {
  ...
  constructor ({ guid = uuidv4(), collectionid = null, gc = true, gcFilter = () => true, meta = null, autoLoad = false, shouldLoad = true } = {}) {
    ...
    const provideSyncedPromise = () => create$4(resolve => {
      /**
...
const createObfuscator = ({ formatting = true, subdocs = true, yxml = true } = {}) => {
  let i = 0;
  const mapKeyCache = create$8();
  const nodeNameCache = create$8();
  const formattingKeyCache = create$8();
  const formattingValueCache = create$8();
      ...
      case Item: {
        ...
        switch (content.constructor) {
          ...
          case ContentType: {
            if (yxml) {
              ...
              if (type instanceof YXmlElement) {
                type.nodeName = setIfUndefined$1(nodeNameCache, type.nodeName, () => 'node-' + i);
              }
              if (type instanceof YXmlHook) {
                type.hookName = setIfUndefined$1(nodeNameCache, type.hookName, () => 'hook-' + i);
              }
            }
            break
          }
          ...
          case ContentFormat: {
            ...
            if (formatting) {
              c.key = setIfUndefined$1(formattingKeyCache, c.key, () => i + '');
              c.value = setIfUndefined$1(formattingValueCache, c.value, () => ({ i }));
            }
            break
          }
          case ContentJSON: {
        ...
        if (item.parentSub) {
          item.parentSub = setIfUndefined$1(mapKeyCache, item.parentSub, () => i + '');
        }
        i++;
        return block
      }
...
const lengthExceeded = () => create$5('Length exceeded!');
...
export { createDecoder as A, map$1 as B, min as C, Doc as D, pow as E, length$1 as F, ObservableV2 as G, isNode as H, Observable as O, UndoManager as U, create$5 as a, all as b, create$4 as c, applyUpdate as d, encodeStateAsUpdate as e, fromBase64 as f, toBase64 as g, createUint8ArrayFromArrayBuffer as h, offChange as i, create$7 as j, encodeStateVector as k, writeVarUint8Array as l, readVarUint as m, writeVarString as n, onChange as o, readVarString as p, getUnixTime as q, readVarUint8Array as r, setIfUndefined$1 as s, transact as t, floor as u, varStorage as v, writeVarUint as w, equalityDeep as x, createEncoder as y, toUint8Array as z };
//# sourceMappingURL=yjs-CvNgPRai.js.map
